      ...
      (let ((word (vector-ref prog cur)))
        (if (= 0 ign)
            (cond
              ...
              ((equal? 'or word) (main prog (+ 1 cur)
                                       (cons (if (or (not (= 0 (car stack))) (not (= 0 (cadr stack))))
                                                 -1
                                                 0) (cddr stack)) ret sl 0))
              ((equal? 'drop word) (main prog (+ 1 cur) (cdr stack) ret sl 0))
              ((equal? 'swap word) (main prog (+ 1 cur) (cons (cadr stack)
                                                              (cons (car stack) (cddr stack))) ret sl 0))
              ...
              ((equal? 'rot word) (main prog (+ 1 cur)
                                        (cons (caddr stack)
                                              (cons (cadr stack)
                                                    (cons (car stack) (cdddr stack)))) ret sl 0))
              ((equal? 'depth word) (main prog (+ 1 cur) (cons (length stack) stack) ret sl 0))
              ((equal? 'define word) (main prog (+ 1 cur) stack ret
                                           (cons (list (vector-ref prog (+ 1 cur)) (+ 2 cur)) sl) 2))
              ((equal? 'if word) (main prog (+ 1 cur) (cdr stack) ret sl (if (= 0 (car stack))
                                                                             1
                                                                             0)))
              ((equal? 'else word) (main prog (+ 1 cur) stack ret sl 1))
              ...
              ((equal? 'while word) (main prog (+ 1 cur) (cdr stack) (cons cur ret) sl (if (= 0 (car stack))
                                                                                           3
                                                                                           0)))
              ((equal? 'wend word) (main prog (car ret) stack (cdr ret) sl 0))
              ((equal? 'repeat word) (main prog (+ 1 cur) stack (cons cur ret) sl 0))
              ((equal? 'until word) (main prog (if (not (= 0 (car stack)))
                                                   (+ 1 cur)
                                                   (car ret)) (cdr stack) (cdr ret) sl 0))
              ((equal? 'tail word) (main prog (cadr (assoc (vector-ref prog (+ 1 cur)) sl)) stack ret sl 0))
